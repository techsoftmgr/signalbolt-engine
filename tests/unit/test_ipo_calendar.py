...
def test_get_ipo_calendar_splits_and_maps(monkeypatch):
    ...
    def fake_fetch(params):
        if params.get("ipo_status") == "pending":
            return [
                {"ticker": "AAA", "issuer_name": "Alpha", "listing_date": "2026-06-20",
                 "lowest_offer_price": 10, "highest_offer_price": 12, "ipo_status": "pending",
                 "primary_exchange": "XNAS"},
                {"ticker": "BBB", "issuer_name": "Beta", "listing_date": None,
                 "lowest_offer_price": None, "highest_offer_price": None, "ipo_status": "pending"},
            ]
        return [
    ...
    # upcoming: dated first, TBD (None date) last
    assert [u["ticker"] for u in out["upcoming"]] == ["AAA", "BBB"]
    assert out["upcoming"][0]["price_low"] == 10.0 and out["upcoming"][0]["price_high"] == 12.0
