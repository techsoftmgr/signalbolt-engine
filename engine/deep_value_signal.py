...
_MAX_FIRES          = 15      # cap per run — a basket, not the whole list
        ...
        try:
            ...
            row = {
                ...
                "ai_explanation": (
                    ...
                    f"turn — not mid-freefall. Multi-month hold; scale in, don't lump-sum. "
    ...
    if fired:
        try:
            push.send_admin_alert(
                title=f"💎 Deep-value window OPEN — {len(fired)} fired",
                body=f"Market {regime.get('off_high_pct')}% off highs. Quality+turn names: "
                     + ", ".join(fired[:10]),
                data={"signal": "deep_value", "count": len(fired)},
            )
        except Exception:
            pass
